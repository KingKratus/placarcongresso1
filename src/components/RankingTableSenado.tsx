...
interface RankingTableSenadoProps {
  analises: Analise[];
}
...
export function RankingTableSenado({ analises }: RankingTableSenadoProps) {
  ...
  useEffect(() => {
    ...
    supabase.from("deputy_performance_scores").select("*").eq("casa", "senado").eq("ano", ano)
      .order("score_total", { ascending: false }).limit(200)
  ...
  const top10Threshold = perfList.length > 0
    ? Number(perfList[Math.floor(perfList.length * 0.1)]?.score_total ?? 0)
    : 0;
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Top 10 — Score de Desempenho (Senado)
              {perfList.length > 0 && (
          ...
          <CardContent className="space-y-2">
            {top10Perf.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
                Nenhum score calculado para o Senado. Acione no painel Admin ou aguarde o cron diário.
              </p>
            ) : (
              top10Perf.map((p, i) => (
            ...
            <CardContent className="space-y-2">
              {top10.map((a, i) => (
                <div key={a.id} className="flex items-center gap-3 p-2 rounded-lg bg-governo/5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{a.senador_nome}</p>
                    <p className="text-[10px] text-muted-foreground">{a.senador_partido} - {a.senador_uf}</p>
                  </div>
